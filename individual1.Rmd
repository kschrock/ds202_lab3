...
# nothappy<- dat %>%
#   filter(happiness != "Very happy" & happiness != "Pretty happy")
# nothappyFindings <- data.frame(aggregate(nothappy$happiness ~ nothappy$maritalStatus, data=nothappy, FUN = function(x){NROW(x)}))


print(happy)
# print(nothappy)
print(happyFindings)
# print(nothappyFindings)
...
ggplot(data=happyPartyAffiliation, aes(x=politicalParty, y=maritalStatus)) +
  ...
  theme_minimal() + labs(title="Happiness based on Marriage Status", x ="Marriage Status", y= "Count of Marriage Status") +coord_flip()
```
